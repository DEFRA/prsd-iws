...
BEGIN
	...
	SELECT @submittedDate = ChangeDate
	 FROM (
		...
		LEFT JOIN 
			[ImportNotification].[NotificationAssessment] NA ON NA.NotificationApplicationId = N.Id
		LEFT JOIN 
			[ImportNotification].[NotificationStatusChange] NSC ON NSC.NotificationAssessmentId = NA.Id
		WHERE 
			N.Id = @notificationId AND NSC.NewStatus IN (2, 14)
		ORDER BY 
			NSC.ChangeDate DESC
		) AS DATA;
	...
	FROM (
		...
		FROM
			[ImportNotification].[Notification] N
		INNER JOIN 
			[Draft].[Import] S ON S.ImportNotificationId = N.Id AND S.[Type] = 'EA.Iws.Core.ImportNotification.Draft.Shipment'
		LEFT JOIN 
			[ImportNotification].[InterimStatus] I ON I.ImportNotificationId = N.Id
		WHERE 
			N.Id = @notificationId
		GROUP BY
			N.Id,
	...
	WHERE
		...
		ps.ValidFrom <= @submittedDate AND 
	...
	BEGIN
		...
		SELECT TOP 1 Price 
			...
			WHERE WasteCategoryTypeId IN (
				...
				WHERE n.id = @notificationId) AND CompetentAuthority = @competentAuthority AND ValidFrom <= @submittedDate
		...
		SET @wasteComponentFees = (SELECT TOP 1 SUM(Price) FROM [Lookup].[PricingFixedFee] WHERE WasteComponentTypeId IN (
			...
			WHERE n.id = @notificationId) AND ValidFrom <= @submittedDate
			GROUP BY Price, ValidFrom
			ORDER BY ValidFrom DESC);
