...
ADD NotificationChargeDate DATE NULL;


/****** Object:  UserDefinedFunction [Notification].[GetPricingInfo]    Script Date: 12/03/2026 09:12:51 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		IF @selfEnteringData = 0
		BEGIN
			SET @price += (@numberOfShipments * (SELECT TOP 1 [Price] FROM [Lookup].[SystemSettings] WHERE CompetentAuthority = 2 AND PriceType = 6 ORDER BY ValidFrom DESC))
		END
	END;
...
/****** Object:  StoredProcedure [Notification].[uspUpdateExportNotificationKeyDates]    Script Date: 12/03/2026 12:29:15 ******/
SET ANSI_NULLS ON
GO
