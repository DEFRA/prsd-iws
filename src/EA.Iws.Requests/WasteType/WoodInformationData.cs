namespace EA.Iws.Requests.WasteType
{
    ...
    public class WoodInformationData : ChemicalCompositionData, IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                yield return
                    new ValidationResult("Please enter a Min and Max concentration for " + EnumHelper.GetDescription(WasteInformationType));
            }
            ...
            if (IsDecimal(MinConcentration) && IsDecimal(MaxConcentration))
            {
                ...
                {
                    yield return
                        new ValidationResult("Max concentration should be in range from 0 to 100 for  " + EnumHelper.GetDescription(WasteInformationType));
                }
                if (minConcentrationValue > maxConcentrationValue)
                {
                    yield return
                        new ValidationResult("Min concentration should be in range from 0 to 100 for  " + EnumHelper.GetDescription(WasteInformationType));
                }
            }
        }
