namespace EA.Iws.DataAccess.Repositories.Reports
{
    ...
    internal class FreedomOfInformationRepository : IFreedomOfInformationRepository
    {
        ...
        public async Task<IEnumerable<FreedomOfInformationData>> GetFOIReport(DateTime from, DateTime to)
        {
            var query = @"SELECT DISTINCT
                                [NotificationNumber],
                                [ImportOrExport],
                            ...
                            FROM 
                                [Reports].[FreedomOfInformationCache]
                            WHERE 
                                [CompetentAuthorityId] = @competentAuthority AND [ReceivedDate] BETWEEN @from AND @to
                            GROUP BY
                                [NotificationNumber],
                                [ImportOrExport],
                                [IsInterim],
            ...
            return await context.Database.SqlQuery<FreedomOfInformationData>(string.Format(query),
              new SqlParameter("@from", from),
              new SqlParameter("@to", to),
